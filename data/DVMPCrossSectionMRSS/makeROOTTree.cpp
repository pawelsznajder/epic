...
int main(int argc, char *argv[]) {
	...
	//create
	TNtuple tree("xsections", "", "xB:t:Q2:xs0:xs1");

	//loop
	std::vector<std::vector<double> >::const_iterator it;

	std::cout << "info: " << __func__ << ": conversion from mb to nb done automatically" << std::endl;
	std::cout << "info: " << __func__ << ": xB = Q2 / (Q2 + MJPsi2) xP" << std::endl;

	double mJPsi2 = pow(3.0969, 2); 

	for (it = grid.begin(); it != grid.end(); it++) {
		tree.Fill(it->at(0) * (it->at(1)/(it->at(1) + mJPsi2)), -1*it->at(2), it->at(1), it->at(3)*1E6, it->at(4)*1E6);
	}
