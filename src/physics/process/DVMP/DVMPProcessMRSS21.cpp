#include "../../../../include/physics/process/DVMP/DVMPProcessMRSS21.h"
...
const std::string DVMPProcessMRSS21::PARAMETER_NAME_GRID_FILE = "grid_file";

const unsigned int DVMPProcessMRSS21::classId =
        BaseObjectRegistry::getInstance()->registerBaseObject(
                new DVMPProcessMRSS21("DVMPProcessMRSS21"));

/*--------------------------------------- Constructors ---------------------------------*/

DVMPProcessMRSS21::DVMPProcessMRSS21(const std::string &className) :
        DVMPProcessModule(className), m_printOutsideGridWarnings(true) {

    //mark that this module does not need CFF one
    setIsCCFModuleDependent(false);

    //paths
    m_gridFile = "data/DVMPCrossSectionMRSS/tables_MRSS.root";
...
DVMPProcessMRSS21::~DVMPProcessMRSS21() {
}

DVMPProcessMRSS21::DVMPProcessMRSS21(const DVMPProcessMRSS21& other) :
        DVMPProcessModule(other), m_printOutsideGridWarnings(true) {
...
DVMPProcessMRSS21* DVMPProcessMRSS21::clone() const {
    return new DVMPProcessMRSS21(*this);
}

void DVMPProcessMRSS21::configure(const ElemUtils::Parameters &parameters) {

    //run for mother
    DVMPProcessModule::configure(parameters);

    if (parameters.isAvailable(DVMPProcessMRSS21::PARAMETER_NAME_GRID_FILE)) {
...
void DVMPProcessMRSS21::initModule() {
...
void DVMPProcessMRSS21::isModuleWellConfigured() {

    //check mother class
    DVMPProcessModule::isModuleWellConfigured();
}

const std::string& DVMPProcessMRSS21::getGridFile() const {
    return m_gridFile;
}

void DVMPProcessMRSS21::setGridFile(const std::string& gridFile) {
    m_gridFile = gridFile;
}

void DVMPProcessMRSS21::loadGrids() {
...
std::shared_ptr<NDInterpolator_3_ML> DVMPProcessMRSS21::loadGrid(
        size_t xsIndex) {
...
void DVMPProcessMRSS21::checkAndSetRange(std::pair<double, double>& target,
...
bool DVMPProcessMRSS21::checkRange(double& value,
...
int DVMPProcessMRSS21::findIndex(const std::vector<double>& vec,
        double val) const {
...
bool DVMPProcessMRSS21::checkIfUnique(const std::vector<double>& vec,
        double val) const {
...
PhysicalType<double> DVMPProcessMRSS21::CrossSection() {

    //check meson type
    if (m_mesonType != MesonType::JPSI) {
        throw ElemUtils::CustomException(getClassName(), __func__,
