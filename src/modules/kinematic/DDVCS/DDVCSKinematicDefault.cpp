/*
 * DDVCSKinematicDefault.cpp
 *
 *  Created on: Feb 12, 2021
 *      Author: Pawel Sznajder (NCBJ)
 */

#include "../../../../include/modules/kinematic/DDVCS/DDVCSKinematicDefault.h"
...
const unsigned int DDVCSKinematicDefault::classId =
        PARTONS::BaseObjectRegistry::getInstance()->registerBaseObject(
                new DDVCSKinematicDefault("DDVCSKinematicDefault"));

DDVCSKinematicDefault::DDVCSKinematicDefault(const std::string &className) :
        DDVCSKinematicModule(className) {
}

DDVCSKinematicDefault::DDVCSKinematicDefault(const DDVCSKinematicDefault &other) :
        DDVCSKinematicModule(other) {
}

DDVCSKinematicDefault::~DDVCSKinematicDefault() {
}

DDVCSKinematicDefault *DDVCSKinematicDefault::clone() const {
    return new DDVCSKinematicDefault(*this);
}

bool DDVCSKinematicDefault::checkIfValid(
        const ExperimentalConditions &conditions, const DDVCSKinematic &kin) {

    //run for mother
    if (!DDVCSKinematicModule::checkIfValid(conditions, kin))
        return false;
...
Event DDVCSKinematicDefault::evaluate(const ExperimentalConditions &conditions,
    ...
    Particle gamma_GP = gammaStar_TAR;
    Particle p_GP = p_TAR;

    //boost
    TVector3 boost_TAR_to_GP = (gammaStar_TAR.getFourMomentum()
    ...
    if (m_decayType != ParticleType::ELECTRON
        ...
        throw ElemUtils::CustomException(getClassName(), __func__,
                ElemUtils::Formatter() << "DDVCSParticles: " << __func__
    ...
    Particle lepton1_EXC(m_decayType, pS_EXC.getMomentum().Unit(),
            sqrt(Q2Prim) / 2.);
    ...
    //check
//    std::cout << "CHECK t: " <<  (pS_LAB.getFourMomentum() - p_LAB.getFourMomentum()).Mag2() << ' ' << t << std::endl;
//    std::cout << "CHECK: y: " <<  (p_LAB.getFourMomentum() * gammaStar_LAB.getFourMomentum()) / (p_LAB.getFourMomentum() * e_LAB.getFourMomentum()) << ' ' << kin.getY() << std::endl;
//    std::cout << "CHECK: Q'2: " << (lepton1_LAB.getFourMomentum()+lepton2_LAB.getFourMomentum()).Mag2() << ' ' << kin.getQ2Prim() << std::endl;
//    std::cout << "CHECK: Q2: " << -1*(e_LAB.getFourMomentum()-eS_LAB.getFourMomentum()).Mag2() << ' ' << kin.getQ2() << std::endl;
